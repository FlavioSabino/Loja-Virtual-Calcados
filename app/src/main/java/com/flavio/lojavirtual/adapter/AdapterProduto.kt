package com.flavio.lojavirtual.adapter
...
import com.flavio.lojavirtual.activities.DetalhesProduto.DetalhesProduto
import com.flavio.lojavirtual.databinding.ProdutoItemBinding
...
class AdapterProduto(val context: Context, val lista_produtos: MutableList<Produto>):
    ...
    override fun onBindViewHolder(holder: ProdutoViewHolder, position: Int) {
       Glide.with(context).load(lista_produtos.get(position).foto).into(holder.fotoPrpduto)
    ...
    inner class ProdutoViewHolder(binding: ProdutoItemBinding) : RecyclerView.ViewHolder(binding.root) {
        val fotoPrpduto = binding.fotoProduto
